...
class ZPushAdmin {
     ...
     */
     ...
     static private $UserDeviceStatus;
    ...
    static public function GetUserDeviceStatus() {
        if (isset(self::$UserDeviceStatus))
            ...
            return SYNC_COMMONSTATUS_SUCCESS;
    }
     ...
     *                            otherwise returns SYNC_COMMONSTATUS_USERDISABLEDFORSYNC, SYNC_COMMONSTATUS_DEVICEBLOCKEDFORUSER, SYNC_COMMONSTATUS_MAXDEVICESREACHED
     * @access public
     */
    static public function GetUserDevicePermission($user, $devid) {
        ...
        if (!$userList) {
            ZLog::Write(LOGLEVEL_DEBUG, "ZPushAdmin::GetUserDevicePermission(): AuthorizedUsersAndDevices file not found - no user restrictions");
            self::$UserDeviceStatus = SYNC_COMMONSTATUS_SUCCESS;
        ...
        $newDeviceAuthorize = true;
        $newUserAuthorize = true;
        $userMaxDevices = 1;
        ...
        for ($i=0;$i<count($lines);$i++)  {
            ...
            if ($user == trim($userIdStatus[0])) {
                ...
                for ($j=1;$j<count($userDeviceArray);$j++) {
                    ...
                    if ($devid == trim( $deviceIdStatus[0])) {
                        $deviceFound = true;
        ...
        if (self::$UserDeviceStatus == SYNC_COMMONSTATUS_SUCCESS) {
            if (! $deviceFound ) {
